...
import {
  ...
  Database, ChevronDown, Pause, Home,
} from "lucide-react";
...
// ─── Sidebar ──────────────────────────────────────────────────────────────────
function Sidebar({ screen, setScreen, onHome }: { screen: Screen; setScreen: (s: Screen) => void; onHome: () => void }) {
  ...
  return (
    <aside style={{ width: 220, flexShrink: 0, background: C.s1, display: "flex", flexDirection: "column", height: "100%", position: "relative", zIndex: 2 }}>
      ...
      <div style={{ padding: "0 12px 12px" }}>
        <button onClick={() => setScreen("capture")} style={{ width: "100%", padding: "12px 8px", background: grad, border: "none", borderRadius: C.r, cursor: "pointer", fontFamily: C.font, fontWeight: 800, fontSize: "0.65rem", color: "#131313", letterSpacing: "0.1em", marginBottom: 8 }}>
          NEW DIAGNOSTIC
        </button>
        <button onClick={onHome} style={{ width: "100%", padding: "9px 8px", background: "none", border: `1px solid ${C.s3}`, borderRadius: C.r, cursor: "pointer", fontFamily: C.font, fontWeight: 700, fontSize: "0.62rem", color: C.sub, letterSpacing: "0.06em", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 8 }}>
    ...
    <div style={{ ...dotBg, flex: 1, overflowY: "auto", padding: "32px 28px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 240px", gap: 12, marginBottom: 24 }}>
        <div style={{ background: C.s1, padding: "28px 28px 24px", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", right: -8, bottom: -8, opacity: 0.04 }}><Activity size={180} color={C.txt} /></div>
            ...
            {[
              ...
              <div key={i} onClick={() => setScreen("analyzing")} style={{ background: C.s1, padding: "14px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, background: C.s2, display: "flex", alignItems: "center", justifyContent: "center" }}><BarChart2 size={16} color={C.orange} /></div>
                  <div>
...
function CaptureScreen({ setScreen }: { setScreen: (s: Screen) => void }) {
  ...
  const bars = Array.from({ length: 22 }, (_, i) => ({ delay: i * 0.08, h: 20 + Math.random() * 60 }));
    ...
    <div style={{ ...dotBg, flex: 1, overflowY: "auto", padding: "28px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 220px", gap: 12 }}>
        ...
        <div style={{ background: C.s1, padding: "16px" }}>
          ...
          {[{ label: "Manufacturer", val: "HEAVY-DYNAMICS CORP" }, { label: "Model", val: "V12-TURBO_COMPRESSOR" }, { label: "Production Year", val: "2022 [REV B]" }, { label: "Serial Number", val: "#DX-8849-AF-001" }].map(({ label, val }) => (
          ...
          <div style={{ background: C.s1, padding: "16px", borderLeft: `3px solid ${C.orange}`, flex: 1 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {CHECKLIST.map((c, i) => {
                ...
                const isQueued = i >= step;
                return (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, opacity: isQueued ? 0.3 : 1 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "180px 1fr 220px", gap: 8 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button style={{ flex: 1, padding: "13px 16px", background: C.s2, border: "none", borderRadius: C.r, cursor: "pointer", fontFamily: C.font, fontWeight: 800, fontSize: "0.72rem", color: C.sub, letterSpacing: "0.06em" }}>ASK EXPERT</button>
          </div>
        </div>
  ...
  return (
    <div style={{ ...dotBg, flex: 1, overflowY: "auto", padding: "28px" }}>
      <div style={{ marginBottom: 12 }}>
        ...
        <h1 style={{ ...H, fontSize: "1.7rem", textTransform: "uppercase", lineHeight: 1.1 }}>HYDRAULIC PUMP <span style={{ color: C.light }}>SEAL REPLACEMENT</span></h1>
      </div>
      ...
      <div style={{ background: `${C.orange}18`, borderLeft: `4px solid ${C.orange}`, padding: "12px 16px", display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12 }}>
        ...
        <div>
          ...
          <p style={{ fontFamily: C.font, fontSize: "0.72rem", color: C.sub, lineHeight: 1.6 }}>HIGH PRESSURE SYSTEM. Ensure hydraulic lines are fully depressurized and the system is locked out / tagged out (LOTO) before beginning disassembly.</p>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 8 }}>
        ...
        <div style={{ background: C.s1, padding: "20px 20px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <p style={{ ...LABEL, color: C.sub, marginBottom: 14 }}>Step {activeStep + 1} of {STEP_CONTENT.length}: {s.sub}</p>
            ...
            <div style={{ marginTop: 16, padding: "10px 12px", background: C.s0, borderLeft: `2px solid ${C.light}` }}>
              <p style={{ fontFamily: C.font, fontStyle: "italic", fontSize: "0.68rem", color: C.sub, lineHeight: 1.6 }}>
                <span style={{ color: C.light }}>Foreman Tip: </span>Work methodically around the entire component. Do not force any step — resistance indicates incomplete disassembly.
              </p>
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {STEP_CONTENT.map((_, i) => <div key={i} style={{ width: i === activeStep ? 24 : 8, height: 3, background: i === activeStep ? C.orange : C.s3, transition: "all 0.3s", cursor: "pointer" }} onClick={() => setActiveStep(i)} />)}
        </div>
        <button onClick={() => { if (activeStep < STEP_CONTENT.length - 1) setActiveStep(s => s + 1); else setScreen("parts"); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 18px", background: grad, border: "none", borderRadius: C.r, cursor: "pointer", fontFamily: C.font, fontWeight: 800, fontSize: "0.7rem", color: "#131313", letterSpacing: "0.06em" }}>
          {activeStep < STEP_CONTENT.length - 1 ? "NEXT STEP" : "PARTS HUB"} <ArrowRight size={12} />
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: C.bg, fontFamily: C.font, overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <Sidebar screen={screen} setScreen={setScreen} onHome={() => setLocation("/")} />

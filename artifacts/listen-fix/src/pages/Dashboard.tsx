...
import {
  ...
  Star, ArrowRight, ArrowLeft, Bell, User, TrendingUp,
  Database, ChevronDown, Pause, Home, X,
} from "lucide-react";
...
const CSS = `
  ...
  .db-capture-upload { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
  .db-analyze-grid { display:grid; grid-template-columns:1fr 240px; gap:8px; margin-top:8px; }
  .db-report-grid { display:grid; grid-template-columns:180px 1fr 220px; gap:8px; }
  ...
  .db-parts-sourcing { display:grid; grid-template-columns:1fr 1fr; gap:2px; background:${C.s0}; margin:0 2px 2px; padding:2px; }
  ...
  @media(max-width:768px){
    ...
    .db-topbar-title { font-size:0.8rem !important; }
    ...
    .db-bottom-nav-item { display:flex; flex-direction:column; align-items:center; gap:3px; padding:8px 12px; background:none; border:none; cursor:pointer; flex:1; }

    .db-home-hero { grid-template-columns:1fr; }
    .db-home-grid { grid-template-columns:1fr; gap:16px; }
    .db-capture-grid { grid-template-columns:1fr; }
    .db-capture-upload { grid-template-columns:1fr 1fr; }
    .db-analyze-grid { grid-template-columns:1fr; }
    .db-report-grid { grid-template-columns:1fr; }
    .db-guide-grid { grid-template-columns:1fr; }
    .db-guide-meta { grid-template-columns:repeat(2,1fr); }
    .db-parts-stats { grid-template-columns:1fr 1fr 1fr; }
    .db-parts-sourcing { grid-template-columns:1fr; }

    .db-screen-pad { padding:20px 16px; }
    .db-hero-h { font-size:1.5rem !important; }
    .db-analyze-h { font-size:1.3rem !important; }
    .db-report-h { font-size:1.4rem !important; }
  }

  @media(max-width:480px){
    .db-capture-upload { grid-template-columns:1fr; }
    .db-guide-meta { grid-template-columns:1fr 1fr; }
  ...
  return (
    <header className="db-topbar">
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <button onClick={onMenuToggle} style={{ padding: 6, background: "none", border: "none", cursor: "pointer", color: C.sub, display: "flex", alignItems: "center" }}>
          <BarChart2 size={16} />
        </button>
        ...
        <span className="db-topbar-title" style={{ fontFamily: C.font, fontWeight: 800, fontSize: "0.9rem", color: C.txt, letterSpacing: "0.05em" }}>LISTEN & FIX</span>
      </div>
...
// ─── Sidebar ──────────────────────────────────────────────────────────────────
function Sidebar({ screen, setScreen, isOpen, onClose, onHome }: {
  screen: Screen; setScreen: (s: Screen) => void; isOpen: boolean; onClose: () => void; onHome: () => void;
}) {
  ...
  const go = (k: Screen) => { setScreen(k); onClose(); };
  return (
    <>
      ...
      <aside className={`db-sidebar ${isOpen ? "open" : ""}`}>
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, padding: "0 0 8px" }}>
          {items.map(({ key, icon, label }) => {
            ...
            return (
              <button key={key} onClick={() => go(key)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: active ? C.s2 : "transparent", border: "none", cursor: "pointer", borderLeft: active ? `3px solid ${C.orange}` : "3px solid transparent", color: active ? C.orange : C.sub, fontFamily: C.font, fontWeight: active ? 700 : 500, fontSize: "0.8rem", textAlign: "left", transition: "all 0.15s" }}>
                {icon} {label}
              </button>
            );
          })}
...
function BottomNav({ screen, setScreen }: { screen: Screen; setScreen: (s: Screen) => void }) {
  const items = [
    ...
    { key: "guide" as Screen,  icon: <Wrench size={20} />,       label: "Guide" },
    { key: "parts" as Screen,  icon: <ShoppingCart size={20} />, label: "Parts" },
  ];
  return (
    <div className="db-bottom-nav">
      {items.map(({ key, icon, label }) => {
        const active = screen === key;
        return (
          <button key={key} onClick={() => setScreen(key)} className="db-bottom-nav-item" style={{ color: active ? C.orange : C.sub, borderTop: active ? `2px solid ${C.orange}` : "2px solid transparent" }}>
            {icon}
...
function HomeScreen({ setScreen }: { setScreen: (s: Screen) => void }) {
  return (
    <div style={{ ...dotBg, padding: "24px 20px" }} className="db-screen-pad">
      <div className="db-home-hero">
        <div style={{ background: C.s1, padding: "24px 24px 20px", position: "relative", overflow: "hidden" }}>
          ...
          <h1 className="db-hero-h" style={{ ...H, fontSize: "2rem", lineHeight: 1.05, textTransform: "uppercase", marginBottom: 10 }}>ENGINEERING<br />DIAGNOSTICS</h1>
        ...
        <div style={{ background: C.s3, padding: "20px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          <div>
            ...
            <p style={{ ...H, fontSize: "1.8rem" }}>$4,280</p>
          </div>
      ...
      <div className="db-home-grid">
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {[
              ...
              <div key={i} onClick={() => setScreen("analyzing")} style={{ background: C.s1, padding: "14px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                  <div style={{ width: 32, height: 32, background: C.s2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><BarChart2 size={15} color={C.orange} /></div>
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontFamily: C.font, fontWeight: 700, fontSize: "0.72rem", color: C.txt, textTransform: "uppercase", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.name}</p>
                    <p style={{ fontFamily: C.font, fontSize: "0.62rem", color: C.sub, marginTop: 2 }}>{d.tech}</p>
                  </div>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <span style={{ fontFamily: C.mono, fontSize: "0.52rem", padding: "2px 5px", background: d.sevColor + "22", color: d.sevColor }}>{d.sev.toUpperCase()}</span>
                  <div style={{ marginTop: 6, width: 80, height: 3, background: C.s3 }}>
                    <div style={{ height: "100%", width: `${d.pct}%`, background: grad }} />
                  </div>
                  <p style={{ ...LABEL, marginTop: 3 }}>{d.pct}%</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div style={{ background: C.s1, padding: "18px" }}>
            <p style={{ fontFamily: C.font, fontSize: "0.75rem", color: C.sub, lineHeight: 1.6, marginBottom: 14 }}>Automated scheduling based on your equipment runtime telemetry.</p>
            <div style={{ display: "flex", gap: 4 }}>
              {[{ date: "OCT 24", name: "OIL FILTER SYNC", time: "08:00 AM" }, { date: "OCT 28", name: "GASKET INSPECTION", time: "14:30 PM" }].map((e, i) => (
                <div key={i} style={{ flex: 1, background: C.s2, padding: "10px 10px", borderLeft: i === 0 ? `2px solid ${C.orange}` : `2px solid ${C.s3}` }}>
                  <p style={{ ...LABEL, color: i === 0 ? C.orange : C.sub, marginBottom: 4 }}>{e.date}</p>
                  <p style={{ fontFamily: C.font, fontWeight: 700, fontSize: "0.68rem", color: C.txt }}>{e.name}</p>
...
function CaptureScreen({ setScreen }: { setScreen: (s: Screen) => void }) {
  ...
  const bars = Array.from({ length: 18 }, (_, i) => ({ delay: i * 0.08 }));
  useEffect(() => {
    if (!recording) return;
    ...
    <div style={{ ...dotBg, padding: "24px 20px" }} className="db-screen-pad">
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          <div style={{ height: 2, width: 24, background: C.light }} />
          <p style={{ ...LABEL, color: C.light }}>Live Session</p>
        </div>
        <h1 style={{ ...H, fontSize: "clamp(1.4rem, 5vw, 2rem)", textTransform: "uppercase" }}>Capture Signature</h1>
      </div>
      <div className="db-capture-grid">
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ background: C.s2, padding: "20px", minHeight: 200, position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            ...
            <p style={{ position: "absolute", top: 12, right: 14, fontFamily: C.mono, fontSize: "1.2rem", color: C.light }}>{fmt(time)}</p>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 70, width: "100%", justifyContent: "center" }}>
              {bars.map((b, i) => (
                <div key={i} style={{ width: 6, background: C.light, opacity: 0.4 + (i%3)*0.2, borderRadius: "1px 1px 0 0", animation: recording ? `waveBar 1.4s ease-in-out ${b.delay}s infinite alternate` : "none", height: recording ? undefined : "20%" }} />
              ))}
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
              <button onClick={() => { setRecording(r => !r); if (!recording) setTimeout(() => setScreen("analyzing"), 3000); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 20px", background: recording ? C.s3 : grad, border: "none", cursor: "pointer", fontFamily: C.font, fontWeight: 800, fontSize: "0.7rem", color: recording ? C.txt : "#131313", letterSpacing: "0.08em" }}>
                {recording ? <><Pause size={13} /> PAUSE</> : <><Mic size={13} /> RECORD</>}
              </button>
              <button onClick={() => setScreen("analyzing")} style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 16px", background: C.s3, border: "none", cursor: "pointer", fontFamily: C.font, fontWeight: 800, fontSize: "0.7rem", color: C.sub, letterSpacing: "0.08em" }}>
                <Play size={13} /> ANALYZE
              </button>
            </div>
          </div>
          <div className="db-capture-upload">
            {[{ icon: <Video size={18} color={C.sub} />, label: "Thermal Video", hint: "Drop .MOV or .MP4" }, { icon: <Mic size={18} color={C.sub} />, label: "Equipment Photos", hint: "Multi-Angle" }].map(({ icon, label, hint }) => (
              <div key={label} style={{ background: C.s1, padding: "12px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <p style={{ ...LABEL, color: C.light, fontSize: "0.55rem" }}>{label.toUpperCase()}</p>
                  {icon}
                </div>
                <div style={{ aspectRatio: "16/9", background: C.s0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", border: `1px dashed ${C.s3}` }}>
                  <Plus size={16} color={C.s3} style={{ marginBottom: 4 }} />
                  <p style={{ ...LABEL, fontSize: "0.52rem" }}>{hint}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ background: C.s1, padding: "16px" }}>
          ...
          {[{ label: "Manufacturer", val: "HEAVY-DYNAMICS CORP" }, { label: "Model", val: "V12-TURBO_COMPRESSOR" }, { label: "Year", val: "2022 [REV B]" }, { label: "Serial #", val: "#DX-8849-AF-001" }].map(({ label, val }) => (
            <div key={label} style={{ marginBottom: 12 }}>
              <p style={{ ...LABEL, marginBottom: 4 }}>{label}</p>
              <div style={{ background: C.s0, padding: "8px 10px", fontFamily: C.mono, fontSize: "0.65rem", color: C.txt, borderLeft: `2px solid ${C.s3}` }}>{val}</div>
            </div>
          ))}
          <div style={{ marginTop: 8, background: C.s0, padding: "10px 12px", borderLeft: `2px solid ${C.orange}` }}>
            ...
            <p style={{ fontFamily: C.font, fontSize: "0.68rem", color: C.sub, lineHeight: 1.6 }}>Place sensor within 20cm of the primary bearing housing. Avoid ambient fan noise.</p>
          </div>
        </div>
      </div>
    </div>
...
function AnalyzingScreen({ setScreen }: { setScreen: (s: Screen) => void }) {
  ...
  useEffect(() => {
    const iv = setInterval(() => setPct(p => { if (p >= 100) { clearInterval(iv); return 100; } return Math.min(p+1.1,100); }), 70);
    return () => clearInterval(iv);
  }, []);
  useEffect(() => {
    const iv = setInterval(() => {
      setLogLines(l => { if (l.length >= LOGS.length) { clearInterval(iv); return l; } return [...l, LOGS[l.length]]; });
    ...
    <div style={{ ...dotBg, padding: "24px 20px" }} className="db-screen-pad">
      <div style={{ marginBottom: 8 }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 8, gap: 12 }}>
          <h1 className="db-analyze-h" style={{ ...H, fontSize: "1.8rem", textTransform: "uppercase", fontStyle: "italic", lineHeight: 1 }}>
            Performing <span style={{ color: C.light }}>Auto-Dex</span> Analysis
          </h1>
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            <p style={{ ...LABEL, color: C.light, marginBottom: 2 }}>Status</p>
            <p style={{ fontFamily: C.mono, fontSize: "0.9rem", color: C.txt }}>0x7F4B-92</p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
          ...
          <p style={{ fontFamily: C.font, fontWeight: 800, fontSize: "1.2rem", color: C.light }}>{Math.round(pct)}%</p>
        </div>
      </div>
      <div className="db-analyze-grid">
        <div style={{ background: C.s0, aspectRatio: "16/10", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: 0.15 }}>
            <div style={{ width: 200, height: 200, borderRadius: "50%", border: `2px solid ${C.light}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ width: 130, height: 130, borderRadius: "50%", border: `2px dashed ${C.orange}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ width: 60, height: 60, borderRadius: "50%", background: C.s3 }} />
              </div>
            </div>
          </div>
          <div style={{ position: "absolute", top: "30%", left: 0, right: 0, height: 1, background: `${C.light}40` }} />
          <div style={{ position: "absolute", top: "18%", left: "20%", width: 90, height: 80, border: `1px solid ${C.light}60` }}>
            <div style={{ position: "absolute", top: -18, left: 0, background: C.light, padding: "2px 6px" }}>
              <p style={{ fontFamily: C.mono, fontSize: "0.48rem", color: C.s0 }}>VIBRATION_PEAK</p>
            </div>
          </div>
          <div style={{ position: "absolute", bottom: "20%", right: "12%", width: 110, height: 60, border: `1px solid ${C.orange}60` }}>
            <div style={{ position: "absolute", bottom: -18, right: 0, background: C.orange, padding: "2px 6px" }}>
              <p style={{ fontFamily: C.mono, fontSize: "0.48rem", color: "#131313" }}>ACOUSTIC_ANOMALY</p>
            </div>
          </div>
          <div style={{ position: "absolute", top: 10, right: 10, background: "rgba(53,53,52,0.85)", backdropFilter: "blur(8px)", padding: "10px 12px", border: `1px solid ${C.s3}` }}>
            <p style={{ ...LABEL, color: C.light, marginBottom: 6 }}>Live Metrics</p>
            {[{ k: "Temp", v: "184.2°C" }, { k: "RPM", v: "32,440" }, { k: "Load", v: "82.1%" }].map(({ k, v }) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 16, marginBottom: 3 }}>
                <p style={{ ...LABEL }}>{k}</p>
                <p style={{ fontFamily: C.mono, fontSize: "0.62rem", color: C.txt }}>{v}</p>
              </div>
            ))}
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ background: C.s1, padding: "14px", borderLeft: `3px solid ${C.orange}`, flex: 1 }}>
            <p style={{ fontFamily: C.font, fontWeight: 800, fontSize: "0.7rem", color: C.txt, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>Process Checklist</p>
            {CHECKLIST.map((c, i) => {
              const isDone = i < step-1, isActive = i === step-1, isQueued = i >= step;
              return (
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: 12, opacity: isQueued ? 0.3 : 1 }}>
                  <div style={{ width: 18, height: 18, flexShrink: 0, marginTop: 1, display: "flex", alignItems: "center", justifyContent: "center", background: isDone ? C.light : "transparent", border: isDone ? "none" : isActive ? `1px solid ${C.orange}` : `1px solid ${C.s3}`, transition: "all 0.3s" }}>
                    {isDone && <CheckCircle2 size={11} color="#131313" />}
                    {isActive && <div style={{ width: 6, height: 6, borderRadius: "50%", background: C.orange, animation: "pulse 1s ease-in-out infinite" }} />}
                  </div>
                  <div>
                    <p style={{ fontFamily: C.font, fontWeight: 600, fontSize: "0.72rem", color: C.txt }}>{c.label}</p>
                    <p style={{ ...LABEL, color: isDone ? C.light : isActive ? C.orange : C.sub, marginTop: 2 }}>{c.desc}</p>
                  </div>
                </div>
              );
            })}
          </div>
          <div ref={logRef} style={{ background: C.s0, padding: "10px", fontFamily: C.mono, fontSize: "0.56rem", lineHeight: 1.8, color: C.sub, maxHeight: 130, overflowY: "auto", scrollbarWidth: "none" }}>
            {logLines.map((l, i) => <div key={i} style={{ color: l.startsWith(">") ? C.sub : C.light }}>{l}</div>)}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button onClick={() => setScreen("home")} style={{ flex: 1, padding: "12px", background: C.s2, border: "none", cursor: "pointer", fontFamily: C.font, fontWeight: 700, fontSize: "0.7rem", color: C.sub, letterSpacing: "0.06em" }}>CANCEL</button>
        <button onClick={() => setScreen("report")} style={{ flex: 2, padding: "12px", background: done ? grad : C.s3, border: "none", cursor: done ? "pointer" : "default", fontFamily: C.font, fontWeight: 700, fontSize: "0.7rem", color: done ? "#131313" : C.sub, letterSpacing: "0.06em", transition: "all 0.5s" }}>
    ...
    <div style={{ ...dotBg, padding: "24px 20px" }} className="db-screen-pad">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16, gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <h1 className="db-report-h" style={{ ...H, fontSize: "1.7rem", textTransform: "uppercase", lineHeight: 1.1 }}>HVAC COMPRESSOR<br />AUDITORY ANOMALY</h1>
        </div>
        <div style={{ background: C.s2, padding: "12px 16px", textAlign: "center", minWidth: 110, flexShrink: 0 }}>
          <p style={{ ...LABEL, marginBottom: 4 }}>Confidence</p>
          <p style={{ fontFamily: C.font, fontWeight: 900, fontSize: "2rem", color: C.orange }}>0.89</p>
          <div style={{ height: 2, background: C.s3, marginTop: 6, position: "relative", overflow: "hidden" }}>
      ...
      <div className="db-report-grid">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ background: C.s1, padding: "14px 12px", borderLeft: `3px solid ${C.orange}` }}>
            ...
            <span style={{ background: C.orange, color: "#131313", fontFamily: C.mono, fontSize: "0.56rem", padding: "3px 8px", fontWeight: 700 }}>Medium — High</span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {[
              ...
              <div key={name} style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  ...
                  <p style={{ fontFamily: C.mono, fontSize: "0.6rem", color: pct > 50 ? C.orange : C.sub, fontWeight: 700 }}>{pct}% LIKELY</p>
                </div>
          ...
          <div style={{ background: C.s0, padding: "14px 16px", flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
              ...
              <p style={{ ...LABEL }}>RAG Retrieved Manual Snippets</p>
            </div>
            {[
              { ref: "Section 4.2: Bearing Maintenance", text: '"When grinding noises exceed 60dB at a 1-meter range, immediate shutdown is required."' },
              { ref: "Technical Bulletin TB-2023-A",     text: '"Models from 2021-2022 show increased sensitivity to vibration-induced heat."' },
            ].map(({ ref, text }) => (
              <div key={ref} style={{ background: C.s1, padding: "10px 12px", marginBottom: 8 }}>
                <p style={{ fontFamily: C.mono, fontSize: "0.56rem", color: C.orange, marginBottom: 6 }}>{ref.toUpperCase()}</p>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button style={{ flex: 1, padding: "13px 12px", background: C.s2, border: "none", cursor: "pointer", fontFamily: C.font, fontWeight: 800, fontSize: "0.7rem", color: C.sub }}>ASK EXPERT</button>
          </div>
        </div>

        {/* Right visual (hidden on mobile via grid-template-columns:1fr) */}
        <div style={{ background: C.s2, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "flex-end", minHeight: 180 }}>
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: 0.1 }}>
            <div style={{ width: 140, height: 140, borderRadius: "50%", border: `4px solid ${C.light}` }}>
              <div style={{ margin: 18, height: "calc(100% - 36px)", borderRadius: "50%", border: `2px dashed ${C.orange}` }} />
            </div>
          </div>
...
function GuideScreen({ setScreen }: { setScreen: (s: Screen) => void }) {
  ...
  const s = STEP_CONTENT[activeStep];

  return (
    <div style={{ ...dotBg, padding: "24px 20px" }} className="db-screen-pad">
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
          <span style={{ background: C.s2, padding: "3px 10px", fontFamily: C.mono, fontSize: "0.56rem", color: C.sub, borderLeft: `2px solid ${C.orange}` }}>INDUSTRIAL-X SERIES</span>
          <span style={{ fontFamily: C.font, fontSize: "0.68rem", color: C.sub }}>REF: HYD-772-B</span>
        </div>
        <h1 style={{ ...H, fontSize: "clamp(1.2rem, 4vw, 1.6rem)", textTransform: "uppercase", lineHeight: 1.1 }}>HYDRAULIC PUMP <span style={{ color: C.light }}>SEAL REPLACEMENT</span></h1>
      </div>

      <div className="db-guide-meta">
        {[{ k: "Skill Level", v: "Intermediate" }, { k: "Duration", v: "45 MIN" }, { k: "Component", v: "PUMP A1" }, { k: "Status", v: "In Progress" }].map(({ k, v }) => (
      ...
      <div style={{ background: `${C.orange}18`, borderLeft: `4px solid ${C.orange}`, padding: "12px 16px", display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12 }}>
        ...
        <div>
          <p style={{ fontFamily: C.font, fontWeight: 800, fontSize: "0.68rem", color: C.light, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>Mandatory Safety Protocol</p>
          <p style={{ fontFamily: C.font, fontSize: "0.7rem", color: C.sub, lineHeight: 1.6 }}>HIGH PRESSURE SYSTEM. Ensure hydraulic lines are fully depressurized and system is locked out / tagged out (LOTO) before beginning.</p>
        </div>
      </div>
      ...
      <div className="db-guide-grid">
          ...
          <div style={{ background: C.s1, padding: "14px 12px", borderBottom: `2px solid ${C.orange}` }}>
            ...
            {tools.map((t, i) => (
              <label key={t} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, cursor: "pointer" }} onClick={() => setChecked(c => c.map((v, j) => j===i?!v:v))}>
                ...
                <span style={{ fontFamily: C.font, fontSize: "0.68rem", color: checked[i]?C.txt:C.sub, textDecoration: checked[i]?"line-through":"none", transition: "all 0.2s" }}>{t}</span>
              </label>
            ))}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {STEP_CONTENT.map((st, i) => (
        ...
        <div style={{ background: C.s1, padding: "20px", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", top: 0, right: 8, fontFamily: C.font, fontWeight: 900, fontSize: "6rem", color: `${C.txt}06`, lineHeight: 1, userSelect: "none" }}>{s.n}</div>
          <div style={{ position: "relative" }}>
            ...
            <p style={{ ...LABEL, color: C.sub, marginBottom: 12 }}>Step {activeStep+1} of {STEP_CONTENT.length}: {s.sub}</p>
            <p style={{ fontFamily: C.font, fontSize: "0.78rem", color: C.txt, lineHeight: 1.7, marginBottom: 12 }}>{s.body}</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {s.subs.map((sub, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                  <span style={{ background: C.orange, color: "#131313", fontFamily: C.mono, fontSize: "0.52rem", padding: "2px 5px", fontWeight: 700, flexShrink: 0, marginTop: 1 }}>{i+1}.{activeStep+1}</span>
                  <p style={{ fontFamily: C.font, fontSize: "0.7rem", color: C.sub, lineHeight: 1.6 }}>{sub}</p>
                </div>
              ))}
            </div>
            <div style={{ marginTop: 16, padding: "10px 12px", background: C.s0, borderLeft: `2px solid ${C.light}` }}>
              <p style={{ fontFamily: C.font, fontStyle: "italic", fontSize: "0.68rem", color: C.sub, lineHeight: 1.6 }}>
                <span style={{ color: C.light }}>Foreman Tip: </span>Work methodically. Resistance indicates incomplete disassembly — do not force.
              </p>
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {STEP_CONTENT.map((_,i) => <div key={i} style={{ width: i===activeStep?20:7, height: 3, background: i===activeStep?C.orange:C.s3, transition: "all 0.3s", cursor: "pointer" }} onClick={() => setActiveStep(i)} />)}
        </div>
        <button onClick={() => { if (activeStep<STEP_CONTENT.length-1) setActiveStep(s=>s+1); else setScreen("parts"); }} style={{ display: "flex", alignItems: "center", gap: 6, padding: "11px 16px", background: grad, border: "none", cursor: "pointer", fontFamily: C.font, fontWeight: 800, fontSize: "0.68rem", color: "#131313" }}>
          {activeStep<STEP_CONTENT.length-1 ? "NEXT" : "PARTS"} <ArrowRight size={12} />
        </button>
      </div>
    </div>
  );
    ...
    <div style={{ ...dotBg, padding: "24px 20px" }} className="db-screen-pad">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16, gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <h1 style={{ ...H, fontSize: "clamp(1.4rem, 5vw, 1.9rem)", textTransform: "uppercase", lineHeight: 1 }}>REQUIRED<br />COMPONENTS</h1>
        </div>
        <div style={{ background: C.s1, padding: "12px 14px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div>
              ...
              <p style={{ ...LABEL, marginTop: 2 }}>Radius: 25 km</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="db-parts-stats">
        {[{ k: "Global Availability", v: "HIGH" }, { k: "Supply Latency", v: "2.4 DAYS" }, { k: "Alternatives Found", v: "04" }].map(({ k, v }) => (
          <div key={k} style={{ background: C.s1, padding: "12px 14px" }}>
            <p style={{ ...LABEL, marginBottom: 4 }}>{k}</p>
            <p style={{ fontFamily: C.font, fontWeight: 900, fontSize: "clamp(1rem, 4vw, 1.4rem)", color: C.txt, letterSpacing: "-0.02em" }}>{v}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        {PARTS_DATA.map((p, i) => (
          <div key={i} style={{ background: C.s1 }}>
            <button onClick={() => setOpen(open===i?null:i)} style={{ width: "100%", display: "flex", gap: 12, padding: "16px", background: "none", border: "none", cursor: "pointer", textAlign: "left", alignItems: "flex-start", flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 200 }}>
                <div style={{ display: "flex", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                  <span style={{ background: "#1f0a0a", color: C.light, fontFamily: C.mono, fontSize: "0.52rem", padding: "2px 8px" }}>REQUIRED</span>
                  <span style={{ fontFamily: C.mono, fontSize: "0.52rem", color: C.sub }}>SKU: {p.sku}</span>
                </div>
              ...
              <div style={{ display: "flex", gap: 16, alignItems: "center", flexShrink: 0 }}>
                <div style={{ textAlign: "right" }}>
                  <p style={{ ...LABEL, marginBottom: 4 }}>Confidence</p>
                  <p style={{ fontFamily: C.font, fontWeight: 900, fontSize: "1.3rem", color: C.txt }}>{p.conf}%</p>
                </div>
                <div style={{ textAlign: "right" }}>
                  <p style={{ ...LABEL, marginBottom: 4 }}>Price Range</p>
                  <p style={{ fontFamily: C.font, fontWeight: 800, fontSize: "0.9rem", color: C.light }}>${p.from} — ${p.to}</p>
            ...
            {open===i && (
              <div className="db-parts-sourcing">
                <div style={{ background: C.s1, padding: "14px" }}>
                  ...
                  {p.local.map((l, j) => (
                    <div key={j} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
                      <div>
                        ...
                        <span style={{ fontFamily: C.mono, fontSize: "0.52rem", padding: "2px 6px", background: l.stock?"#052e16":"#1f0a0a", color: l.stock?"#4ade80":"#f87171", display: "inline-block", marginTop: 4 }}>
                          {l.stock ? "READY FOR PICKUP" : "BACKORDERED"}
                        </span>
                      </div>
                      <button style={{ padding: "7px 12px", background: C.orange, border: "none", cursor: "pointer", fontFamily: C.font, fontWeight: 700, fontSize: "0.6rem", color: "#131313" }}>DIRECTIONS</button>
                    </div>
                  ))}
                </div>
                <div style={{ background: C.s1, padding: "14px" }}>
                  ...
                  {p.online.map((l, j) => (
                    <div key={j} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
                      ...
                      <button style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px", background: grad, border: "none", cursor: "pointer", fontFamily: C.font, fontWeight: 700, fontSize: "0.6rem", color: "#131313" }}>
